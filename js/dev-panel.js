...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const CARGOS_EQUIPE = [
    {
      id: 'admin',
      label: 'Administrador',
      descricao: 'Portal completo, exceto Configurações e Segurança.',
      tabs: ['dashboard', 'usuarios', 'assinaturas', 'pagamentos', 'avaliacoes', 'planos', 'relatorios']
    },
    {
      id: 'gerente',
      label: 'Gerente',
      descricao: 'Usuários, assinaturas, pagamentos, avaliações, planos e relatórios.',
      tabs: ['dashboard', 'usuarios', 'assinaturas', 'pagamentos', 'avaliacoes', 'planos', 'relatorios']
    },
    {
      id: 'funcionarios',
      label: 'Funcionários',
      descricao: 'Dashboard, usuários e avaliações.',
      tabs: ['dashboard', 'usuarios', 'avaliacoes']
    }
  ];
  ...
  const NIVEIS_PORTAL = [
    {
      id: 'usuario',
      label: 'Usuário comum',
      descricao: 'Sem acesso ao portal do desenvolvedor.'
    },
    {
      id: 'funcionarios',
      label: 'Funcionários',
      descricao: 'Dashboard, usuários e avaliações no portal.'
    },
    {
      id: 'gerente',
      label: 'Gerente',
      descricao: 'Gerencia usuários, assinaturas, pagamentos e relatórios.'
    },
    {
      id: 'admin',
      label: 'Administrador',
  ...
  function portalRoleOf(userOrSession) {
    const src = userOrSession || session;
    let nivel = String(src?.nivelAcesso || src?.nivel_acesso || '')
      .trim()
      .toLowerCase()
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .replace(/[\s-]+/g, '_');
    if (nivel === 'superadmin' || nivel === 'super_administrador') nivel = 'super_admin';
    if (nivel === 'administrador' || nivel === 'administradora') nivel = 'admin';
    if (nivel === 'funcionario' || nivel === 'funcionaria') nivel = 'funcionarios';
    if (['super_admin', 'gerente', 'funcionarios', 'admin'].includes(nivel)) return nivel;
    const cargo = String(src?.cargoDev || src?.cargo_dev || '')
      .trim()
      .toLowerCase();
  ...
  function portalRoleLabelOf(role) {
    const id = String(role || '')
      .trim()
      .toLowerCase();
    if (!id || id === 'usuario') return 'Usuário comum';
    const fromNivel = NIVEIS_PORTAL.find((r) => r.id === id);
    if (fromNivel) return fromNivel.label;
    const fromCargo = CARGOS_EQUIPE.find((c) => c.id === id);
    if (fromCargo) return fromCargo.label;
  ...
  function canManagePortalRoles() {
    return isPortalOwner();
  }

  /** Aprovar pagamento e arquivar/reativar ciclo: só desenvolvedor (super_admin). */
  function canApproveOrArchiveUsers() {
    return isPortalOwner();
  }

  function allowedNavTabs() {
    if (isPortalOwner()) return NAV.map((n) => n.id);
    const role = portalRoleOf(session);
    // Admin e gerente: todos os menus operacionais (só Configurações/Segurança ficam fora)
    if (role === 'admin' || role === 'gerente') {
      return NAV.map((n) => n.id).filter((id) => !OWNER_ONLY_TABS.includes(id));
    }
    const fromSession = Array.isArray(session?.permissoesDev) ? session.permissoesDev : null;
    if (fromSession && fromSession.length) {
      return fromSession.map((t) => String(t)).filter((id) => !OWNER_ONLY_TABS.includes(id));
    }
    const meta = CARGOS_EQUIPE.find((c) => c.id === role);
    return meta ? [...meta.tabs] : ['dashboard'];
  }

  const OWNER_ONLY_TABS = ['configuracoes', 'logs'];

  function isOwnerOnlyTab(tabId) {
    return OWNER_ONLY_TABS.includes(String(tabId || ''));
  }

  function isTabBlocked(tabId) {
    if (isPortalOwner()) return false;
    const tab = String(tabId || '');
    if (isOwnerOnlyTab(tab)) return true;
    const role = portalRoleOf(session);
    if (role === 'admin' || role === 'gerente') return false;
    return !allowedNavTabs().includes(tab);
  }

  function tabBlockedMessage(tabId) {
    if (isOwnerOnlyTab(tabId)) return 'Este menu é restrito ao desenvolvedor.';
    return 'Seu cargo não tem permissão para este menu.';
  }

  function canOpenTab(tabId) {
    return !isTabBlocked(tabId);
  }
  ...
  function cargoLabelOf(cargoOrUser) {
    return portalRoleLabelOf(
      typeof cargoOrUser === 'object' ? portalRoleOf(cargoOrUser) : cargoOrUser
    );
  }

  function cargoBadgeHTML(u) {
    const label = portalRoleLabelOf(portalRoleOf(u));
    if (!label || label === 'Usuário comum') return '<span class="pas-muted">—</span>';
    return `<span class="pas-cargo-pill">${escape(label)}</span>`;
  }
  ...
  function canApprovePayment(status) {
    if (!canOpenTab('pagamentos') && !canOpenTab('assinaturas')) return false;
  ...
  function startUsersLiveUpdates() {
    ...
    // Presença online/offline (15s): necessário para o chat do desenvolvedor.
    ...
    return `
      <button type="button" class="pas-admin-msg__user" role="listitem" data-message-user-id="${escape(String(u.id))}" data-message-tipo="${
        ...
        <span class="pas-presence ${online ? 'is-online' : 'is-offline'}" data-message-presence="${escape(String(u.id))}" title="${
          online ? 'Logado no sistema' : 'Deslogado'
  ...
  function patchMessagePanelPresence(users = usersCache) {
    ...
    panel.querySelectorAll('[data-message-presence]').forEach((el) => {
      ...
      el.title = online ? 'Logado no sistema' : 'Deslogado';
  ...
  function restrictedAreaLabel() {
    if (isPortalOwner()) return 'Desenvolvedor';
    return portalRoleLabelOf(portalRoleOf(session));
  }

  function shellHTML() {
    const photo = avatarSrc(session.photo);
    const roleLabel = isPortalOwner() ? 'Desenvolvedor' : restrictedAreaLabel();
    return `
      <div class="pas-admin">
        <aside class="pas-admin__sidebar" id="dev-sidebar">
          ...
          <nav class="pas-admin__nav" aria-label="Menu do desenvolvedor">
            ${visibleNav()
              .map((item) => {
                const blocked = isTabBlocked(item.id);
                return `
              <button type="button" class="pas-admin__nav-btn${blocked ? ' is-locked' : ''}" data-tab="${item.id}"${
                  blocked ? ` aria-disabled="true" title="${escape(tabBlockedMessage(item.id))}"` : ''
                }>
                <span class="pas-admin__nav-icon" aria-hidden="true">${ICONS[item.icon] || ''}</span>
                <span>${escape(item.label)}</span>
                ${blocked ? '<em class="pas-admin__nav-lock" aria-hidden="true">🔒</em>' : ''}
              </button>`;
              })
              .join('')}
          </nav>
  ...
  function refreshProfileHeader(next) {
    ...
    Object.assign(session, {
      ...
      nivelAcesso: next.nivelAcesso != null ? next.nivelAcesso : session.nivelAcesso,
      cargoDev: next.cargoDev !== undefined ? next.cargoDev : session.cargoDev,
      permissoesDev: Array.isArray(next.permissoesDev) ? next.permissoesDev : session.permissoesDev,
      role:
        next.role != null
          ? next.role
          : isPortalOwner()
    ...
    if (typeof DevAuth !== 'undefined' && DevAuth.setSession) {
      DevAuth.setSession({
        ...session,
        role: isPortalOwner() ? 'developer' : 'equipe'
      });
    }
  ...
  async function approvePayment(userId, onDone, { hasProof = true } = {}) {
    if (!userId) return;
    if (!canApproveOrArchiveUsers()) {
      UI.toast('Somente o desenvolvedor pode aprovar pagamentos.', 'error');
      return;
    }
    const msg = hasProof
  ...
  function paymentRowsHTML(users, { limit = 8, showActions = true, showPresence = false, showCargo = false } = {}) {
    const list = users.slice(0, limit);
    let cols = showPresence ? 8 : 6;
    if (showCargo) cols += 1;
    if (!list.length) {
      return `<tr><td colspan="${cols}" class="pas-empty-cell">Nenhum registro nesta lista.</td></tr>`;
    }
    const canEditCargo = canManagePortalRoles();
    const canApproveArchive = canApproveOrArchiveUsers();
    return list
      .map((u) => {
        const st = API.normalizeStatusPagamento(u.statusPagamento);
        const kind = statusClass(st);
        const okProof = hasPaymentProof(u);
        const showApprove = showActions && canApproveArchive && canApprovePayment(st);
        ...
        return `
          <tr data-user-id="${escape(String(u.id))}" data-has-proof="${okProof ? '1' : '0'}">
            ...
            ${showCargo ? `<td>${cargoBadgeHTML(u)}</td>` : ''}
            ...
            <td class="pas-row-actions">
                ...
                showApprove
                  ...
                  : kind === 'pago'
                    ...
                    : canApproveArchive
                      ...
                      : `<span class="pas-muted" title="Somente o desenvolvedor pode aprovar">Restrito</span>`
              }
              ${
                showPresence && canApproveArchive
              ...
              ${
                showCargo && canEditCargo
                  ? `<button type="button" class="pas-link-btn" data-action="edit-cargo">Nível de acesso</button>`
                  : ''
              }
            </td>
          </tr>`;
  ...
  function bindPaymentTable(scope, onDone) {
    ...
    scope.querySelectorAll('[data-action="ciclo-archive"], [data-action="ciclo-active"]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        if (!canApproveOrArchiveUsers()) {
          UI.toast('Somente o desenvolvedor pode arquivar ou reativar o ciclo.', 'error');
          return;
        }
        const row = btn.closest('[data-user-id]');
        const userId = row?.getAttribute('data-user-id');
    ...
    scope.querySelectorAll('[data-action="edit-cargo"]').forEach((btn) => {
      btn.addEventListener('click', () => {
        const row = btn.closest('[data-user-id]');
        const userId = row?.getAttribute('data-user-id');
        if (!userId) return;
        const user = usersCache.find((u) => String(u.id) === String(userId));
        openCargoPermissionModal(user || { id: userId }, onDone);
      });
    });
  }

  function openCargoPermissionModal(user, onDone) {
    if (!user || !isPortalOwner()) return;
    if (typeof UI === 'undefined' || typeof UI.modal !== 'function') return;
    const current = portalRoleOf(user);
    const name = String(user.name || user.nome || 'Usuário').trim() || 'Usuário';
    const email = String(user.email || '').trim();

    const body = document.createElement('div');
    body.className = 'dev-cargo-modal';
    body.innerHTML = `
      <p class="dev-cargo-modal__intro">
        Defina o nível de acesso de <strong>${escape(name)}</strong>${
          email ? ` <span class="pas-muted">(${escape(email)})</span>` : ''
        }
        na coluna <code>nivel_acesso</code> para liberar o portal do desenvolvedor.
      </p>
      <div class="form-group">
        <label for="dev-cargo-select">Nível de acesso</label>
        <select id="dev-cargo-select">
          ${NIVEIS_PORTAL.map(
            (c) =>
              `<option value="${escape(c.id)}"${current === c.id ? ' selected' : ''}>${escape(c.label)}</option>`
          ).join('')}
        </select>
      </div>
      <div class="dev-cargo-modal__cards" id="dev-cargo-cards">
        ${NIVEIS_PORTAL.map(
          (c) => `
          <button type="button" class="dev-cargo-card${current === c.id ? ' is-selected' : ''}" data-cargo="${escape(c.id)}">
            <strong>${escape(c.label)}</strong>
            <span>${escape(c.descricao)}</span>
          </button>`
        ).join('')}
      </div>
      <p class="pas-muted dev-cargo-modal__hint">
        Super Admin é exclusivo do desenvolvedor (e-mail cadastrado).
        Administrador, Gerente e Funcionários entram no portal com Configurações e Segurança bloqueadas.
      </p>`;

    const selectEl = () => body.querySelector('#dev-cargo-select');
    body.querySelectorAll('[data-cargo]').forEach((card) => {
      card.addEventListener('click', () => {
        const id = card.getAttribute('data-cargo') || '';
        const sel = selectEl();
        if (sel) sel.value = id;
        body.querySelectorAll('.dev-cargo-card').forEach((el) => {
          el.classList.toggle('is-selected', el.getAttribute('data-cargo') === id);
        });
      });
    });
    ...
    UI.modal({
      title: 'Nível de acesso no portal',
      body,
      dialogClass: 'modal-dialog--wide',
      actions: [
        {
        ...
        },
        {
          ...
          className: 'btn-primary',
          onClick: async (close) => {
            const next = String(selectEl()?.value || '').trim().toLowerCase() || 'usuario';
            UI.showLoading('Salvando nível de acesso…');
            try {
              const res = await API.updateUserForDev(user.id, { nivelAcesso: next });
              UI.hideLoading();
              if (!res?.ok) {
              ...
              UI.toast(
                next === 'usuario'
                  ? 'Usuário comum — sem acesso ao portal.'
                  : `Nível ${portalRoleLabelOf(next)} aplicado. O usuário pode entrar em dev-login.html.`,
                'success'
              );
              close();
              if (typeof onDone === 'function') await onDone(res.user);
            } catch (err) {
              UI.hideLoading();
  ...
  function aivenSummaryCardHTML() {
    return `
      <section class="pas-neon-dash" id="pas-neon-dash">
        <article class="pas-card pas-neon-card" id="pas-aiven-db-card">
          <div class="pas-card__head">
            <div>
              <h3>Banco Aiven</h3>
              <p class="pas-muted" style="margin:0.25rem 0 0">PostgreSQL de produção — database PowerAppsSistemas</p>
            </div>
            <div class="pas-neon-head-meta">
              <span class="${neonToneClass('normal')}">${neonStatusDot('normal')} Ativo</span>
            </div>
          </div>
          <p class="pas-neon-quota-line"><strong>Serviço:</strong> powerapps-sistemas</p>
          <p class="pas-neon-quota-line"><strong>Plano:</strong> Free (SSL obrigatório)</p>
          <p class="pas-neon-foot form-hint">O card de limite Neon foi removido — o sistema usa somente Aiven.</p>
          <div class="pas-neon-actions">
            <a class="pas-ghost-btn pas-neon-link-btn" href="https://console.aiven.io" target="_blank" rel="noopener noreferrer">Abrir console Aiven</a>
          </div>
        </article>
      </section>`;
  }

  function neonSummaryCardHTML(_usage) {
    return aivenSummaryCardHTML();
  }
  ...
  function replaceNeonSummaryCard(content, usage) {
    ...
    if (dash) {
      dash.outerHTML = aivenSummaryCardHTML();
    }
      ...
      content.innerHTML = `
        <section class="pas-welcome pas-welcome--compact">
          <div>
            ...
            <p>${
              isPortalOwner()
                ? 'Veja quem está online, converse com usuários Pro, confirme pagamentos e defina o nivel_acesso (admin, gerente, funcionários).'
                : 'Veja quem está online/offline, converse com usuários Pro e confirme pagamentos.'
            }</p>
          </div>
        ...
        <article class="pas-card">
          <div class="pas-table-wrap">
            <table class="pas-table">
              ...
              <tbody>
                ${paymentRowsHTML(users, { limit: 200, showPresence: true, showCargo: true })}
              </tbody>
            </table>
          </div>
        </article>`;
  ...
  function enrichRatingsWithUserPhotos(ratings = []) {
    ...
    return (ratings || []).map((r) => {
      ...
      if (!u) return r;
      const next = { ...r };
      if (u.name) next.userName = u.name;
      if (u.email) next.userEmail = u.email;
      if (!next.photo && u.photo) next.photo = u.photo;
      return next;
    });
  }
  ...
  async function renderTab(opts = {}) {
    ...
    if (!canOpenTab(activeTab)) {
      activeTab = allowedNavTabs()[0] || 'dashboard';
    }
    setActiveNav();
  ...
  document.addEventListener('pas:dev-go-tab', (e) => {
    ...
    if (!tab) return;
    if (!canOpenTab(tab)) {
      UI.toast(tabBlockedMessage(tab), 'error');
      return;
    }
    activeTab = tab;
    renderTab();
    ...
    .then((res) => {
      if (res?.ok && res.profile) {
        refreshProfileHeader({
          ...
          nivelAcesso: res.profile.nivelAcesso || session.nivelAcesso,
          cargoDev: res.profile.cargoDev,
          permissoesDev: res.profile.permissoesDev,
          role: canManagePortalRoles() ? 'developer' : 'equipe',
          loggedAt: session.loggedAt || new Date().toISOString()
        });
        if (activeTab === 'usuarios') renderTab({ quiet: true });
      }
    })
    .catch(() => {});
  ...
  if (/configuracoes|sec-cloudflare/i.test(hash) && canOpenTab('configuracoes')) {

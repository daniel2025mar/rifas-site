 ...
 * Login → API Express → tabela usuarios.senha no Aiven (bcrypt.compare no servidor).
 */
const DevAuth = (() => {
  ...
  async function login({ email, password }) {
    ...
    const session = {
      ...
      role:
        ...
        String(result.session.nivelAcesso || '').toLowerCase() === 'super_admin'
          ? 'developer'
          : 'equipe',
      isDev:
        result.session.isDev === true ||
      ...
      nivelAcesso: result.session.nivelAcesso || 'usuario',
      cargoDev: result.session.cargoDev || null,
      permissoesDev: Array.isArray(result.session.permissoesDev)

...
const API = (() => {
  ...
  async function request(method, path, body) {
    ...
    try {
      ...
      return { ok: false, error: 'Erro inesperado no servidor.' };
    } catch (err) {
      return {
        ok: false,
  ...
  const SESSION_VALIDATION_CACHE_MS = 55 * 1000;
  ...
  function isUserOnline(user, nowMs = Date.now()) {
    if (!user || !user.sessionActive) return false;
    const raw = user.lastSeen || user.sessionAt;
    if (!raw) return false;
    const t = new Date(raw).getTime();
    if (!Number.isFinite(t)) return false;
    return nowMs - t <= ONLINE_MS;
  }
  ...
  function getAccessLevel(userOrEmail) {
    ...
    if (typeof userOrEmail === 'object') {
      if (userOrEmail.isDev === true || userOrEmail.portal === 'dev') return 'super_admin';
      const from = String(userOrEmail.nivelAcesso || userOrEmail.nivel_acesso || '')
        .trim()
        .toLowerCase();
      if (from === 'super_admin' || from === 'superadmin') return 'super_admin';
  ...
  function hasMinAccessLevel(userOrEmail, required) {
    const rank = { usuario: 1, admin: 2, super_admin: 3 };
  ...
  /**
   * Login do portal dev: senha no Neon; privilégio só se a API marcar super_admin.
   */
  async function loginDeveloperPortal({ email, password } = {}) {
    ...
    const nivel = String(result.session?.nivelAcesso || result.session?.nivel_acesso || '')
      .trim()
      .toLowerCase();
    if (nivel !== 'super_admin' && nivel !== 'superadmin') {
      try {
        Store.clearSession?.();
      } catch {
        /* ignore */
      ...
      return {
        ok: false,
        error: 'Acesso negado. Esta conta não tem permissão de desenvolvedor.'
      };
    }
  ...
  async function listSupportThreads({ limit = 100 } = {}) {
    const res = await request('GET', `/api/suporte/threads?limit=${encodeURIComponent(limit)}`);
  ...
  async function getSupportLive(userId) {
    const res = await request('GET', `/api/suporte/live/${encodeURIComponent(userId)}`);
    if (!res.ok) {
    ...
    return {
      ...
      typingUser: !!res.typingUser,
      typingDev: !!res.typingDev,
      signature: res.signature || messages.map((m) => `${m.id}:${m.read ? 1 : 0}`).join('|')
    };
  }

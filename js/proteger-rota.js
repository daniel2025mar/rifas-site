...
const ProtecaoRota = (() => {
   ...
   * skipAge: não aplica gate de confirmação de idade (PF)
   */
  const ROTAS = {
    ...
    'termos.html': { nivel: NIVEIS.USUARIO },
    ...
    'historico.html': { nivel: NIVEIS.USUARIO },
    ...
    'confirmar-idade.html': { nivel: NIVEIS.USUARIO, skipPayment: true, skipAge: true },
    'compartilhar.html': { nivel: NIVEIS.USUARIO, hybrid: true },

    'admin-pagamentos.html': { nivel: NIVEIS.ADMIN },
    'dev.html': { nivel: NIVEIS.SUPER_ADMIN, portal: 'dev', skipPayment: true, skipAge: true }
  };
  ...
  function normalizeNivel(value) {
    ...
    if (v === 'administrador' || v === 'administradora' || v === 'admin') return NIVEIS.ADMIN;
    if (v === 'gerente') return NIVEIS.ADMIN;
    if (v === 'funcionarios' || v === 'funcionario' || v === 'funcionaria') return NIVEIS.ADMIN;
  ...
  async function redirectLogin(motivo) {
    try {
   ...
   * @param {boolean} [options.ensureAge=true] — gate idade PF → confirmar-idade.html
  ...
  async function protegerRota(options = {}) {
    ...
    const ensureAge =
      options.ensureAge !== false && !efetiva.skipAge && nivelExigido !== NIVEIS.PUBLICO;

    if (nivelExigido === NIVEIS.PUBLICO) {
      return { ok: true, public: true, page, nivel: NIVEIS.PUBLICO };
    }

    // Portal do desenvolvedor (sessão separada; privilégio via nivelAcesso ou cargo da equipe)
    if (efetiva.portal === 'dev') {
      ...
      const cargoDev = String(devSession?.cargoDev || '')
        .trim()
        .toLowerCase();
      const rawNivel = String(devSession?.nivelAcesso || '')
        .trim()
        .toLowerCase();
      const hasPortalRole =
        rawNivel === 'super_admin' ||
        rawNivel === 'superadmin' ||
        rawNivel === 'admin' ||
        rawNivel === 'gerente' ||
        rawNivel === 'funcionarios' ||
        ['gerente', 'financeiro', 'vendas', 'funcionarios'].includes(cargoDev);
      const isOwner =
        nivelDev === NIVEIS.SUPER_ADMIN ||
        devSession?.isDev === true ||
        rawNivel === 'super_admin' ||
        rawNivel === 'superadmin';
      if (!isOwner && !hasPortalRole) {
        try {
          DevAuth.logout?.();
        } catch {
          /* ignore */
      ...
      const nivel = isOwner ? NIVEIS.SUPER_ADMIN : NIVEIS.ADMIN;
      if (!temPermissao(nivel, nivelExigido) && !hasPortalRole) {
        if (redirect) window.location.replace(DEV_LOGIN_URL);
      ...
      return {
        ...
        session: {
          ...devSession,
          portal: 'dev',
          nivelAcesso: isOwner ? 'super_admin' : rawNivel || 'usuario',
          cargoDev: cargoDev || null
        },
        nivel,
        page
      };
    ...
    if (!localSessionOk()) {
      if (redirect) await redirectLogin('auth');
    ...
    if (validateRemote) {
      ...
      if (!remote?.ok) {
        ...
        if (!softFail) {
          if (redirect) {
            await redirectLogin(
              remote?.reason === 'replaced' ? 'outro-dispositivo' : 'sessao'
            );
          }
    ...
    if (ensureAge && typeof API !== 'undefined' && typeof API.ensureAgeAccess === 'function') {
      const age = await API.ensureAgeAccess();
      if (!age?.ok) {
        return { ok: false, reason: age?.reason || 'age', session, nivel, page };
      }
    }
